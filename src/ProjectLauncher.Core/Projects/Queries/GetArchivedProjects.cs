...
public sealed record GetArchivedProjectsQuery
    : IQuery<Result<IReadOnlyList<ProjectResponse>>>;

public sealed class GetArchivedProjectsQueryHandler(IProjectStore projectStore)
{
    public async Task<Result<IReadOnlyList<ProjectResponse>>> HandleAsync(
        GetArchivedProjectsQuery query,
        CancellationToken cancellationToken = default)
    {
        try
        {
            var projects = await projectStore.GetArchivedAsync(cancellationToken);
